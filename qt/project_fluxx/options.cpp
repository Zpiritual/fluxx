...
{
    QList<QListWidgetItem*> profiles{};
    // Get all profile names
    for(int i = 0; i < user_profiles->count(); i++)
    {
        profiles.append(user_profiles->item(i));
    }
    // Check if input profile name already exist
    for(int i = 0; i < profiles.size(); i++)
    {
        if (profiles.at(i)->text() == profile_name)
            return true;
    }
    return false;
}
...
void Options::initializeWidgets()
{
